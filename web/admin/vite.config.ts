...
export default defineConfig(({ mode }) => {
  // No VITE_ prefix: this value configures only the development
  // server and is never exposed through import.meta.env or the
  // production bundle.
  const env = loadEnv(mode, process.cwd(), 'KNIEVEL_ADMIN_');
  const apiOrigin = env.KNIEVEL_ADMIN_API_ORIGIN || 'http://localhost:8080';
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        '^/admin/config\\.json$': { target: apiOrigin, changeOrigin: true },
      },
    },
    test: {
      globals: true,
